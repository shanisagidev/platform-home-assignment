...
const REMEDIATION_NOTE = 'Investigating potential shadow IT usage. Initiating remediation process.';
...
test.describe('Verify Alert lifecycle: Manual Remediation', () => {
  test('should complete full alert remediation flow', async ({ page, loginPage, policiesPage, alertsPage }) => {
    // --- Login ---
    await page.goto('/');
    ...
    await policiesPage.clickStartNewScan();
    await expect(policiesPage.getScanCompletedText(), "verify scan is completed").toBeVisible({ timeout: 120_000 });

    // --- Navigate to Alerts ---
    await page.goto('/alerts');
    await expect(alertsPage.alertsPageSection, "verify alert page is opened").toBeVisible();
    ...
    await expect(alertsPage.alertsTable, "verify alert list is visible").toBeVisible();
    ...
    await expect(await alertsPage.getAlertStatus()).toContainText('In Progress');
    ...
    await expect(await alertsPage.getAlertStatus()).toContainText('Awaiting User Verification', { timeout: 300_000 });

    // --- Change status → Resolved ---
    await alertsPage.changeAlertStatus('Resolved');

    await expect(await alertsPage.getAlertStatus(), "verify status is changed to Resolved").toContainText('Resolved');

    // --- Add comment ---
    await alertsPage.addComment(COMMENT_TEXT);

    // --- Verify comment is visible ---
    await expect(await page.getByText(COMMENT_TEXT), "verify comment is added").toBeVisible();
  ...
  test.afterEach(async ({ page, upperNavbar }) => {
    await page.goto('/');
    await upperNavbar.resetTestData();
  });
