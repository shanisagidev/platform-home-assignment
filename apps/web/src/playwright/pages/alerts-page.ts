...
export class AlertsPage {
  ...
  async changeAlertStatus(newStatus: string) {
    await this.drawer
      .getByRole("button", { name: "Change alert status" })
      .click();
    await this.page.getByRole("option", { name: newStatus }).click();
  }

  getAlertStatus() {
    const statusButton = this.drawer.getByRole("button", {
  ...
  getAlertAssignee() {
    const assigneeButton = this.drawer.getByRole("button", { name: "Assign alert" });
  ...
  async waitForStatus(expectedStatus: string, timeoutMs = 300_000) {
    await expect
      .poll(
        async () => {
          if (!(await this.drawer.isVisible())) return "";
          return (await this.getAlertStatus().textContent()) ?? "";
        },
